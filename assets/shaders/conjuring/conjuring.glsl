...
float Bu(vec2 pos){
 	float a = capsuleLength(pos + vec2(0.0, -0.5), vec2(1.0, 0.9));   
...
{
    vec2 uv = (fragCoord.xy - u_resolution * 0.5) / u_resolution.yy * 20.0;
    ...
    col.r = BurisabaCircle(uv)
        ...
        + 0.005 / abs(boxLength(rotate(uv, M_PI05 * 0.0 - u_time * 0.5)) - 4.5)
        + 0.005 / abs(boxLength(rotate(uv, M_PI05 * 0.25 - u_time * 0.5)) - 4.5)
        + 0.005 / abs(boxLength(rotate(uv, M_PI05 * 0.5 - u_time * 0.5)) - 4.5)
        + 0.005 / abs(boxLength(rotate(uv, M_PI05 * 0.75 - u_time * 0.5)) - 4.5)
